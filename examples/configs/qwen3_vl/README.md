...
32B 的 [兼容 W8 配置](vl/32b/w8a16_legacy.toml) 与 [runtime W8 配置](vl/32b/w8a16.toml) 是不同 checkpoint 合同。兼容格式要求 `example.opt_in.QWEN3_VL_32B_ALLOW_GRAPH_BLOCKED="1"`，不能把它映射到无此 gate 的 runtime 格式；显式 opt-in 不代表质量认证。
...
| `prefill.fast_replay` | 默认复用准备好的 prefill Graph；没有独立 `vision.fast_replay`。 |
