...
transformer blocks and final continuous LayerNorm/projection.  This matches
``nextdit_traj.py`` exactly at the subsystem boundary and retains per-block
numerical probes for correctness runs.
"""
...
class NextDiTRPURuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self._rpu_execution = component_execution(
            rpu_execution,
            NEXTDIT_COMPONENT,
            entry_point="NextDiTRPURuntime",
        )
        self._fmb_execution_component_id = NEXTDIT_COMPONENT
        ...
        # Raw diagnostic exports are disabled in the public runtime.
        self._debug_export = False
        ...
        self._handle: int | None = torch.ops.rpu.internvla_nextdit_create()
        # Keep only native addresses, not the CPU model whose marker points back
        # to this runtime. The original condition/step containers stay mutable.
        self._native_resource = _InstalledNativeResource(
            self, self._handle, torch.ops.rpu.internvla_nextdit_destroy,
            graphs=(self._graph_cache,),
            keepalive=(self._keep, self._self_cache, self._packed_self_cache,
                self._cross_cache, self._conditions, self._steps, self._step_batches,
                self._cross_context_slots, self._step_modulation_slots,
                self._step_final_scale_slots, self._step_activated_slots,
                self._layer_outputs_rpu, self._raw_actions_dummy_rpu,
                self._raw_actions_input_rpu, self._hidden_input_rpu,
                self._packed_hidden_inputs_rpu, self._packed_layer_outputs_rpu),
            label="InternVLA NextDiT", handle_name="_handle")
        self._handle_finalizer = self._native_resource.finalizer
        try:
            self._install_weights(self._handle)
            torch.ops.rpu.internvla_nextdit_set_chunk_envelope(
                self._handle, int(self._packed_self_cache.max_seq_len),
                int(self._cross_cache.max_seq_len), 0)
            traj_dit._rpu_nextdit_runtime = self
        except BaseException as error:
            if self._native_resource.cleanup_failure(error, self):
                if getattr(traj_dit, "_rpu_nextdit_runtime", None) is self:
                    vars(traj_dit)["_rpu_nextdit_runtime"] = None
            raise
    ...
    def _install_weights(self, handle: int) -> None:
        ...
        torch.ops.rpu.internvla_nextdit_set_weights(
            ...
            *tail,
            ...
            self._fold_norm_weights is not None,
        )
        if self._fused_qkq:
            fused_qkq = [
                _col(
    ...
    def _forward_one(
        ...
        *,
        ...
        chunk_size, execution_plan = self._execution_plan(
            action_rows=ACTION_SEQ,
            cross_seq=condition.shape[1],
            use_action_encoder=use_action_encoder,
            populate_cross_cache=populate_cross_cache,
            action_batch=1,
        )
        ...
        with self._graph_cache.capture(sig):
            hidden_rpu = torch.ops.rpu.internvla_nextdit_forward(
                ...
                execution_plan.selected.stage_tuple.physical_descriptor,
            )
        resolved = int(
            torch.ops.rpu.internvla_nextdit_get_resolved_chunk_size(
                self._handle
            )
        )
        if resolved != chunk_size:
            raise RuntimeError(
                "InternVLA NextDiT dry/forward chunk plan drift: "
                f"dry={chunk_size}, forward={resolved}, rows={ACTION_SEQ}"
            )
        publish_execution_receipt(
            self,
            execution_plan,
            component=NEXTDIT_COMPONENT,
            stage="action",
            generation=self._fmb_execution_generation,
            logical_len=ACTION_SEQ,
        ...
        # another trajectory/step can reuse it (P4 stable-DDR rule).
    ...
    def _graph_signature(
        ...
        plan_words = (
            ...
            else tuple(execution_plan.graph_key_words())
        )
        key = (
            cross_seq,
            use_action_encoder,
    ...
    def _execution_plan(
        ...
        *,
        ...
        populate_cross_cache: bool,
        action_batch: int,
        denoise_steps: int = 0,
        prepare_modulation_on_rpu: bool = False,
    ):
        stage = self._rpu_execution.get("action", {})
        requested_chunk = stage.get("chunk_size", "auto")
        box = {}
        execution_len, chunk_size = plan_bounded_prefill_execution(
            ...
            execution_component="nextdit_action",
            execution_stage="action",
            execution_native=("internvla_nextdit", int(self._handle)),
            resolve_stage_domain=lambda rows: (
                torch.ops.rpu.internvla_nextdit_resolve_stage_domain(
                    self._handle,
                    int(rows),
                    int(cross_seq),
            ...
            exact_chunk_size=(
                int(requested_chunk)
                if isinstance(requested_chunk, int) else None
            ),
            graph_mode=GRAPH_COMPOSITE_CHILD,
            request_id="internvla:nextdit_action:action",
            queue_owner_id=int(self._handle),
            physical_metadata=(
                ("execution_generation", int(
                    self._fmb_execution_generation
                )),
                ("self_attention_joint_domain", 1),
                ("cross_attention_ddr_required", 1),
                (
                    "route_reason:PREFIX_HISTORY_DDR_REQUIRED_NO_RAW_RESIDENCY_ABI",
                    1,
                ),
            ),
            plan_result_sink=lambda result: box.__setitem__(
                "result", result
            ),
            plan_signature=(int(cross_seq), bool(use_action_encoder), bool(populate_cross_cache), int(action_batch), int(denoise_steps), bool(prepare_modulation_on_rpu)),
            graph_cache=self._graph_cache,
        )
        result = box["result"]
        if (
            execution_len != int(action_rows)
            or result.selected is None
            or not result.selected.stage_tuple.physical_descriptor
        ):
            raise RuntimeError(
                "InternVLA NextDiT planner returned no native descriptor"
            )
    ...
    def _forward_actions_uniform_batch(
        ...
        action_rows = action_batch * ACTION_SEQ
        chunk_size, execution_plan = self._execution_plan(
        ...
        if resolved != chunk_size:
            raise RuntimeError(
                "InternVLA NextDiT dry/forward chunk plan drift: "
                f"dry={chunk_size}, forward={resolved}, rows={action_rows}"
            )
        publish_execution_receipt(
            self,
            execution_plan,
    ...
    def denoise_actions(
        ...
        *,
        ...
        action_rows = action_batch * ACTION_SEQ
        prepare_modulation_on_rpu = self._rpu_step_prepare and fold_for_unroll
        chunk_size, execution_plan = self._execution_plan(
            action_rows=action_rows,
            cross_seq=condition.shape[1],
            use_action_encoder=True,
            ...
            prepare_modulation_on_rpu=prepare_modulation_on_rpu,
        )
...
def build_nextdit_runtime(
    ...
    *,
    ...
    execution = component_execution(
        rpu_execution,
        NEXTDIT_COMPONENT,
        entry_point="build_nextdit_runtime",
    )
