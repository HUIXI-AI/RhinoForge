"""
QwenPI05 Action Model Converter for RPU (Phase 1: Op-by-Op)
...
def convert_qwenpi05_action_model_for_rpu(
    ...
    *,
    ...
    # Round-5 MED hardening: fail loud on double-conversion. `convert_
    ...
    # weights (Pitfall 1 double-swizzle class). Stamp a per-module flag
    # on the first successful conversion and raise on subsequent
    # invocations. Matches the Qwen3Adapter guard introduced for Phase 2
    # public API; applied here for legacy Phase 3/4 converter entry
    # points called directly from test harnesses and Pi0.5 glue code.
    if getattr(action_expert, "_rpu_swizzled", False) or \
       getattr(action_io, "_rpu_swizzled", False):
        raise RuntimeError(
            ...
            "a second conversion would double-swizzle (P1). Reload the "
    ...
    # Round-5: stamp _rpu_swizzle_started BEFORE mutation so a failure
    ...
    # Round-5: stamp _rpu_swizzled only on full success (after weight
