...
Key differences vs adarms_converter.py:
...
_QWENPI05_ACTION_COMPONENT = "action_expert"
_QWENPI05_EXECUTION_COMPONENTS = {
    _QWENPI05_ACTION_COMPONENT: {"action": ("chunk_size",)},
}


def resolve_qwenpi05_execution(value, *, suffix_len, entry_point):
    """Resolve the unsupported escape hatch through the common config schema."""
    root = normalize_rpu_execution(
        value,
        entry_point=entry_point,
        supported_components=_QWENPI05_EXECUTION_COMPONENTS,
    )
    action = resolve_component_rpu_execution(
        root,
        _QWENPI05_ACTION_COMPONENT,
        entry_point=entry_point,
        supported_components=_QWENPI05_EXECUTION_COMPONENTS,
        profile_auto={"action": {"chunk_size": "auto"}},
    )
    fixed_chunk = ((int(suffix_len) + 15) // 16) * 16
    requested = action["action"]["chunk_size"]
    if isinstance(requested, int) and requested != fixed_chunk:
        raise ValueError(
            f"{entry_point}: action_expert action.chunk_size must be 'auto' "
            f"or exactly {fixed_chunk} for suffix_len={suffix_len}; got "
            f"{requested}. QwenPI05's non-causal suffix is one physical chunk."
        )
    return root, action


def _plan_qwenpi05_action_execution(
    expert,
    *,
    logical_len,
    execution_len,
    ...
    """Resolve the complete native descriptor before Graph admission."""
    action_config = expert._fmb_execution_component_config["action"]
    requested = action_config.get("chunk_size", "auto")
    exact_chunk = requested if isinstance(requested, int) else None
    handle = int(expert._rpu_qwenpi05_handle)
    result_box = {}
    resolved_execution, resolved_chunk = plan_bounded_prefill_execution(
        ...
        execution_component=_QWENPI05_ACTION_COMPONENT,
        execution_stage="action",
        execution_native=("qwenpi05", handle),
        ...
        resolve_stage_domain=lambda length: (
            torch.ops.rpu.qwenpi05_resolve_action_stage_domain(
                handle,
                int(length),
                int(logical_len),
                int(prefix_len),
        ...
        request_id="qwenpi05:action_expert:action",
        graph_mode=GRAPH_COMPOSITE_CHILD,
        queue_owner_id=handle,
        physical_metadata=(
            ("component:action_expert", 1),
            ("execution_generation", int(expert._fmb_execution_generation)),
    ...
    if (
        ...
        raise RuntimeError(
            "QwenPI05 action planner returned no consumable native stage descriptor"
        )
    return result


def _publish_qwenpi05_action_receipt(expert, plan, *, logical_len):
    selected = plan.selected
    if selected is None:
        raise RuntimeError("QwenPI05 action planner selected no plan")
    resolved = int(torch.ops.rpu.qwenpi05_get_resolved_chunk_size(
        expert._rpu_qwenpi05_handle
    ))
    if resolved != selected.stage_tuple.compute_chunk:
        raise RuntimeError(
            "QwenPI05 action dry/forward chunk plan drift: "
            f"dry={selected.stage_tuple.compute_chunk}, forward={resolved}"
        )
    receipt = plan.as_dict(include_candidates=False)
    receipt.update({
        "stage": "action",
        "component": _QWENPI05_ACTION_COMPONENT,
        "generation": int(expert._fmb_execution_generation),
...
def _validate_qwenpi05_reconfigure(expert, config):
    if config != expert._rpu_execution:
        raise ValueError("QwenPI05 has a fixed single-chunk contract; construct a new owner to change configuration")


def _apply_qwenpi05_reconfigure(expert, old, new, generation, *, force_rebuild=False):
    if old != new:
        raise ValueError("QwenPI05 has a fixed single-chunk contract")
    # Private calibration retires actual retained graphs without changing the
    # fixed model configuration. Public same-config updates remain no-ops.
    cache = expert._rpu_qwenpi05_graph_cache
    cache.begin_warmup()
    cache.clear()
    expert._rpu_kv_cache.reset_to_position(0)
    vars(expert).pop("_rpu_last_execution_plan", None)
    vars(expert)["_fmb_execution_generation"] = int(generation)


def _close_qwenpi05_runtime(expert):
    def retire():
        cache = expert._rpu_qwenpi05_graph_cache
        cache.begin_warmup()
        cache.clear()
        finalizer = expert._rpu_qwenpi05_handle_finalizer
        if finalizer.alive:
            torch.ops.rpu.qwenpi05_destroy(expert._rpu_qwenpi05_handle)
            finalizer.detach()
    return expert._execution_session.shutdown(retire)
...
_QWENPI05_RUNTIME_INSTALL_ATTRS = (
    ...
    "_fmb_execution_component_config",
...
def _prepare_cond_dense_weights(expert):
    ...
    for layer_idx, layer in enumerate(expert.layers):
        for src_norm_name, (w_out, b_out) in (
            ...
            # If a previous patch already prepared these buffers, reuse them.
            # Note: the buffer name is versioned (`_rpu_cond_w_rp`, _rp = row partition)
            # so older cached buffers from col-partition / non-folded converter
            # versions (`_rpu_cond_w`, `_rpu_cond_w_gf`) are ignored.
            ...
            # Previously this expanded the weight 8x (.repeat(NUM_CORES, 1)) and used
            # col-partition so every core computed the same [3H] redundantly. The
            # new row-partition path keeps the natural [3H, H] weight and fuses the
            # per-core partial reduction + bias via all_reduce_sum_residual.
            ...
            # Cache on the module (versioned name to avoid colliding with any
            # stale non-folded `_rpu_cond_w` from a previous converter version).
...
def _precompute_mrope_cos_sin(rotary_emb, prefix_len, suffix_len):
    ...
    Root-cause reference: the SPM rope kernel reads cos bytes at
    (position * head_dim/2) stride in units of fp16. Passing full
    head_dim format to it causes every other position to read from
    the "wrong" half of the previous position's row, effectively
    halving the position index and producing cos_sim ~0.85 per head.
...
def _create_kv_cache(config, max_seq_len):
    ...
    Uses RPUCache for allocation per CONTEXT.md decision: RPUCache produces
    the 7D swizzled layout required by C++ insert_kcache/insert_vcache and
    SDPA kernels. Position management and prefix pre-fill are manual.
    """
...
def _prefill_kv_cache_python_swizzle(cache, prefix_key_values, prefix_layer_offset, num_layers):
    """LEGACY: Pre-fill prefix KV into RPUCache by directly writing 7D swizzled format.
...
def _prefill_kv_cache(cache, prefix_key_values, prefix_layer_offset, num_layers):
    ...
    for layer_idx in range(num_layers):
        ...
        torch.rpu.insert_kcache(pk, k_tmp, 0)   # v5: bound on torch.rpu, not rpu_backend
...
def patch_qwenpi05_for_rpu_fused(
    expert,
    *,
    prefix_len=0,
    suffix_len=31,
    ...
    """Install the fixed action runtime; replacement is an exclusive cold operation."""
    session = getattr(expert, "_execution_session", None)
    with session._lock if session is not None else nullcontext():
        if session is not None:
            session.require_cold()
        return _install_qwenpi05_for_rpu_fused(
            expert, prefix_len=prefix_len, suffix_len=suffix_len,
            max_seq_len=max_seq_len, cpu_rotary_emb=cpu_rotary_emb,
            rpu_execution=rpu_execution,
        )


def _install_qwenpi05_for_rpu_fused(
    expert,
    *,
    prefix_len=0,
    suffix_len=31,
    ...
    # ------------------------------------------------------------------ #
    old_session = getattr(expert, "_execution_session", None)
    ...
    existing_execution = getattr(expert, "_rpu_execution", None)
    execution_root, action_execution = resolve_qwenpi05_execution(
        existing_execution if rpu_execution is None else rpu_execution,
        suffix_len=suffix_len,
        entry_point="patch_qwenpi05_for_rpu_fused",
    )
    ...
    # ── Diagnostic timestamps (CLOCK_MONOTONIC, same clock as kernel driver
    # logs) to let you correlate driver `Duration: X us` lines with Python
    # call sites. Compare `[PY-DIAG] ... t=...` prints with the driver's
    # bracketed timestamps (e.g. `[532302.095642]`) to identify which Python
    # block is producing each kernel burst.
    ...
    # ------------------------------------------------------------------ #
    # Step 4: M-RoPE cos/sin pre-computation (CONV-04)
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Step 5b: Move final AdaRMSNorm BACK to CPU (D-402 parity with AdaRMS).
    ...
    #
    #   Fix: keep final norm on CPU for Python-side computation. The
    ...
    def rpu_qwenpi05_fused_forward(
        ...
        *,
        ...
        # Reject outside the existing RoPE envelope before any device transfer.
        actual_suffix_len = suffix_embeds.shape[1]
        ...
        action_plan = _plan_qwenpi05_action_execution(
            self,
            ...
            is_causal=is_causal,
            graph_cache=self._rpu_qwenpi05_graph_cache,
        )
        ...
        try:
            ...
            # SPM cleanup -- ALWAYS, even on error (CONV-05, T-03-04)
        ...
        # -- Final norm in Python (D-402): norm is on CPU fp32 for parity with
        #    CPU reference path. Move output to CPU + cast to fp32 first.
        ...
        _publish_qwenpi05_action_receipt(
            self, action_plan, logical_len=actual_suffix_len
        )
    ...
    try:
        ...
        torch.ops.rpu.qwenpi05_set_chunk_envelope(handle, int(kv_cache.max_seq_len), 0)
        ...
        expert._rpu_execution = execution_root
        expert._fmb_execution_component_id = _QWENPI05_ACTION_COMPONENT
        expert._fmb_execution_component_config = action_execution
        ...
        # Cold replacement keeps the same owner and its immutable callbacks.
        reuse_callbacks = old_session is not None and had_old_handle
        session = bind_execution_session(
            expert, execution_root, entry_point="QwenPI05ActionExpert",
            supported_components=_QWENPI05_EXECUTION_COMPONENTS,
            validate=None if reuse_callbacks else lambda config: _validate_qwenpi05_reconfigure(expert, config),
            apply=None if reuse_callbacks else lambda old, new, generation, *, force_rebuild=False: _apply_qwenpi05_reconfigure(
                expert, old, new, generation, force_rebuild=force_rebuild
            ),
            rollback=None if reuse_callbacks else lambda old, _new, generation: _apply_qwenpi05_reconfigure(
                expert, old, old, generation, force_rebuild=True
            ),
            graph_mode=GRAPH_COMPOSITE_CHILD,
        )
        session._bind_planner_owner(expert, _QWENPI05_ACTION_COMPONENT)
        expert.close_rpu_execution = types.MethodType(_close_qwenpi05_runtime, expert)
