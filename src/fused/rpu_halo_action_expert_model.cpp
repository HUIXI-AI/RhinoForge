...
// Python 装载入口: python/rpu_backend/adapters/halo/。
//
...
// HALO_ACTION_FIXED_KERNEL_BASIS: non-manifest launchers implement the fixed
// row-mask merge, normalization/activation, or BufferDecl transport dictated by
// this exact action geometry; none has a runtime selector.

constexpr int64_t HALO_ACTION_ATTN_SITE = 8691412429184154460LL;
constexpr int64_t HALO_ACTION_KV_SITE = 1056254339788262076LL;
constexpr int64_t HALO_ACTION_ATTN_ALL_REDUCE_SITE =
    5520964670786628241LL;
constexpr int64_t HALO_ACTION_MLP_ALL_REDUCE_SITE =
    4569073412420351464LL;
constexpr int64_t HALO_ACTION_QKV_LINEAR_SITE = 6845166295456518525LL;
constexpr int64_t HALO_ACTION_TEXT_O_LINEAR_SITE = 6819834060407356611LL;
constexpr int64_t HALO_ACTION_ACT_O_LINEAR_SITE = 4838683919682874083LL;
constexpr int64_t HALO_ACTION_GATE_LINEAR_SITE = 5752165231748682072LL;
constexpr int64_t HALO_ACTION_UP_LINEAR_SITE = 6745473868249290233LL;
constexpr int64_t HALO_ACTION_DOWN_LINEAR_SITE = 2600624301715187799LL;
constexpr int64_t HALO_ACTION_Q_ROPE_SITE = 4115106074483039215LL;
constexpr int64_t HALO_ACTION_K_ROPE_SITE = 6071864173885174758LL;
constexpr int64_t HALO_ACTION_PREPARE_ALL_REDUCE_SITE =
    3905443535840784612LL;
constexpr int64_t HALO_ACTION_BIAS_PRELOAD_SITE = 2863746215900564039LL;
constexpr uint32_t HALO_ACTION_KV_CAPABILITIES = KV_INSERT_CAP_V2;
constexpr int64_t HALO_ACTION_KV_REASON_PREFIX_HISTORY_DDR_REQUIRED = 2;

enum class HaloActionRopeRoute : int64_t {
    ROPE_1D_SPM = 1,
};

enum class HaloActionAllReduceRoute : int64_t {
    PREPARE_RING_INPUT = 3,
};

enum class HaloActionMutableDmaRoute : int64_t {
...
// subclass_chunk_size_valid — HALO action attention requires one chunk.
// The row masks and KV merge cover all 18 query rows. Admit only cs >= seq_len
// with num_chunks == 1, in addition to the shared tile and sync constraints.
bool HaloActionExpertModel::subclass_chunk_size_valid(
    int64_t cs, int64_t seq_len, int64_t /*position*/) const {
    return chunk_within_envelope(cs) && cs >= seq_len;
}

void HaloActionExpertModel::set_execution_envelope(int64_t max_kv_len) {
    set_chunk_envelope(max_kv_len, /*fixed C32=*/32);
    set_chunk_size_override(32);
    enable_execution_reconfigure_guard();
}

std::vector<int64_t> HaloActionExpertModel::resolve_stage_domain(
    int64_t prefix_len, int64_t requested_chunk_size) {
    TORCH_CHECK(moe_chunk_size_ > 0,
                "RPU_PLANNER_REJECT:CAPABILITY: HALO action weights are not initialized");
    TORCH_CHECK(prefix_len >= 0,
                "RPU_PLANNER_REJECT:CAPABILITY: HALO action prefix_len must be >= 0");
    TORCH_CHECK(requested_chunk_size == 0 || requested_chunk_size == 32,
                "RPU_PLANNER_REJECT:EXACT_MISMATCH: HALO action supports only "
                "AUTO or its fixed C32 ABI, got ", requested_chunk_size);
    const int64_t seq_len = moe_chunk_size_;
    auto mask_shape = at::zeros(
        {1, 1, seq_len, prefix_len + seq_len},
    ...
    return encode_fmb_prefill_stage_domain(
        resolve_prefill_stage_domain_for_shape(
            seq_len, prefix_len, std::optional<at::Tensor>(mask_shape),
...
HaloActionExpertModel::physical_manifest_for_candidate(
    ...
    TORCH_CHECK(
        ...
        "HALO action COMPLETE KV descriptor requires one query chunk");
    ...
    TORCH_CHECK(
        ...
        "HALO action COMPLETE descriptor requires one compute chunk");
    ...
    append(
        FmbRouteFamily::ATTENTION, HALO_ACTION_ATTN_SITE,
    ...
    for (int64_t invocation = 0; invocation < 6; ++invocation) {
        append_linear(
            HALO_ACTION_QKV_LINEAR_SITE,
            {seq_len, qkv_widths[invocation], h, 1, tp,
             has_qkv_bias_ ? 1 : 0},
            invocation);
    }
    append_linear(
        HALO_ACTION_TEXT_O_LINEAR_SITE,
        {seq_len, h, nq * hd, 0, tp, 0});
    append_linear(
        HALO_ACTION_ACT_O_LINEAR_SITE,
        {seq_len, h, nq * hd, 0, tp, 0});
    for (int64_t invocation = 0; invocation < 2; ++invocation) {
        append_linear(
            HALO_ACTION_GATE_LINEAR_SITE,
            {seq_len, intermediate_size(), h, 1, NUM_CORES, 0},
            invocation);
        append_linear(
            HALO_ACTION_UP_LINEAR_SITE,
            {seq_len, intermediate_size(), h, 1, NUM_CORES, 0},
            invocation);
        append_linear(
            HALO_ACTION_DOWN_LINEAR_SITE,
    ...
    append(
        ...
        HALO_ACTION_PREPARE_ALL_REDUCE_SITE,
        static_cast<int64_t>(
            HaloActionAllReduceRoute::PREPARE_RING_INPUT),
        {seq_len, h, tp, ring_selector});
    append(
        FmbRouteFamily::ALL_REDUCE, HALO_ACTION_ATTN_ALL_REDUCE_SITE,
        ring_selector, {seq_len, h, tp, NUM_CORES});
    append(
        FmbRouteFamily::ALL_REDUCE, HALO_ACTION_MLP_ALL_REDUCE_SITE,
        ring_selector, {seq_len, h, NUM_CORES, NUM_CORES});

    append(
        FmbRouteFamily::ROPE, HALO_ACTION_Q_ROPE_SITE,
        static_cast<int64_t>(HaloActionRopeRoute::ROPE_1D_SPM),
        {cos_sin_start, seq_len, local_q, hd, NUM_CORES});
    append(
        FmbRouteFamily::ROPE, HALO_ACTION_K_ROPE_SITE,
        static_cast<int64_t>(HaloActionRopeRoute::ROPE_1D_SPM),
        {cos_sin_start, seq_len, nkv / tp, hd, NUM_CORES});

    manifest.routes.push_back({
        HALO_ACTION_KV_SITE, FmbRouteFamily::KV_INSERT,
        static_cast<int64_t>(kv_plan.route()),
        HALO_ACTION_KV_REASON_PREFIX_HISTORY_DDR_REQUIRED,
        std::vector<int64_t>(kv_arguments.begin(), kv_arguments.end())});
    if (has_qkv_bias_) {
        const int64_t bias_widths[] = {local_q * hd, local_kv, local_kv};
        for (int64_t invocation = 0; invocation < 3; ++invocation) {
            const int64_t per_core = bias_widths[invocation];
            append(
                FmbRouteFamily::MUTABLE_DMA,
                HALO_ACTION_BIAS_PRELOAD_SITE,
                static_cast<int64_t>(
                    HaloActionMutableDmaRoute::DDR_SCATTER_TO_SPM),
...
HaloActionExpertModel::physical_manifest_forward_capability(
    ...
    int64_t chunk_size)
{
    ...
    //   is/8        : MLP gate/up 槽 (M2 预留; commit-1 在 down 上以全宽 merge)
...
std::vector<BufferDecl> HaloActionExpertModel::declare_buffers(const LayoutContext& lctx) {
    ...
    if (has_qkv_bias_) {
        ...
        add_bias("q_bias_act", dma_safe(local_q * hd),
                 &MoeLayerWeights::q_bias, /*route_invocation=*/0);
        add_bias("k_bias_act", dma_safe(local_kv),
                 &MoeLayerWeights::k_bias, /*route_invocation=*/1);
        add_bias("v_bias_act", dma_safe(local_kv),
                 &MoeLayerWeights::v_bias, /*route_invocation=*/2);
    }
...
void HaloActionExpertModel::build_layer_subgraph(int layer_idx, const ChunkInfo& chunk) {
    ...
    auto qkv = [&](const at::Tensor& w, const char* out, const char* bias_slot,
                   int64_t n, int64_t route_invocation) {
        if (ctx().has_complete_physical_manifest()) {
            ctx().consume_physical_route(
                FmbRouteFamily::LINEAR, HALO_ACTION_QKV_LINEAR_SITE,
                static_cast<int64_t>(FmbLinearRouteSelector::AUTO_TILE),
                /*resolved_flags=*/0,
                {seq_len, n, h, 1, tp, has_qkv_bias_ ? 1 : 0},
                route_invocation);
        }
        rpu_launch_linear_spm_to_spm_acc16_kernel(
            addr(0, "input_norm"), w, addr(0, out),
            seq_len, n, h, /*partition=*/1, /*num_cores=*/tp,
            has_qkv_bias_ ? layer_addr(layer_idx, 0, bias_slot) : 0u, /*scale=*/{});
    };
    qkv(lw.q_w, "q",   "q_bias",     nq * hd,  /*route_invocation=*/0);
    qkv(lw.k_w, "k",   "k_bias",     nkv * hd, /*route_invocation=*/1);
    qkv(lw.v_w, "v",   "v_bias",     nkv * hd, /*route_invocation=*/2);
    qkv(mw.q_w, "q_a", "q_bias_act", nq * hd,  /*route_invocation=*/3);
    qkv(mw.k_w, "k_a", "k_bias_act", nkv * hd, /*route_invocation=*/4);
    qkv(mw.v_w, "v_a", "v_bias_act", nkv * hd, /*route_invocation=*/5);
    ...
    KvInsertSegmentPlan kv_plan = [&] {
        if (!ctx().has_complete_physical_manifest()) {
            return rpu_resolve_kvinsert_segment_plan_auto(
                ctx().position + chunk.offset, seq_len, seq_len,
                tp, nkv, hd, HALO_ACTION_KV_CAPABILITIES);
        }
        const FmbRouteManifestEntry& route = ctx().find_physical_route(
            FmbRouteFamily::KV_INSERT, HALO_ACTION_KV_SITE);
        KvInsertSegmentPlan frozen =
            restore_kvinsert_plan(
                HALO_ACTION_KV_SITE, route.arguments, tp, nkv, hd);
        TORCH_CHECK(
            frozen.logical_rows() == seq_len &&
                frozen.physical_rows() == seq_len &&
                frozen.segment(0).position == ctx().position + chunk.offset,
            "HALO action KV descriptor geometry drift");
        ctx().consume_physical_route(
            FmbRouteFamily::KV_INSERT, HALO_ACTION_KV_SITE,
    ...
    if (ctx().has_complete_physical_manifest()) {
        ctx().consume_physical_attention_route(
            HALO_ACTION_ATTN_SITE, AttentionExecutionPolicy::DDR_KV);
    }
    ...
    if (ctx().has_complete_physical_manifest()) {
        ctx().consume_physical_route(
            FmbRouteFamily::LINEAR, HALO_ACTION_TEXT_O_LINEAR_SITE,
            static_cast<int64_t>(FmbLinearRouteSelector::AUTO_TILE),
            /*resolved_flags=*/0,
            {seq_len, h, nq * hd, 0, tp, 0});
    }
    rpu_launch_linear_spm_to_spm_acc16_kernel(
        ...
        /*bias_spm_addr=*/0, /*scale=*/{});
    if (ctx().has_complete_physical_manifest()) {
        ctx().consume_physical_route(
            FmbRouteFamily::LINEAR, HALO_ACTION_ACT_O_LINEAR_SITE,
            static_cast<int64_t>(FmbLinearRouteSelector::AUTO_TILE),
            /*resolved_flags=*/0,
            {seq_len, h, nq * hd, 0, tp, 0});
    }
    ...
    if (ctx().has_complete_physical_manifest()) {
        ctx().consume_physical_route(
            FmbRouteFamily::ALL_REDUCE,
            HALO_ACTION_ATTN_ALL_REDUCE_SITE, ring_selector,
            /*resolved_flags=*/0,
    ...
    auto mlp_half = [&](const at::Tensor& gate_w, const at::Tensor& up_w,
                        ...
                        const char* dn, int64_t route_invocation) {
        if (ctx().has_complete_physical_manifest()) {
            ctx().consume_physical_route(
                FmbRouteFamily::LINEAR, HALO_ACTION_GATE_LINEAR_SITE,
                static_cast<int64_t>(FmbLinearRouteSelector::AUTO_TILE),
                /*resolved_flags=*/0,
                {seq_len, intermediate_size(), h, 1, NUM_CORES, 0},
                route_invocation);
        ...
        if (ctx().has_complete_physical_manifest()) {
            ctx().consume_physical_route(
                FmbRouteFamily::LINEAR, HALO_ACTION_UP_LINEAR_SITE,
                static_cast<int64_t>(FmbLinearRouteSelector::AUTO_TILE),
                /*resolved_flags=*/0,
                {seq_len, intermediate_size(), h, 1, NUM_CORES, 0},
                route_invocation);
        ...
        if (ctx().has_complete_physical_manifest()) {
            ctx().consume_physical_route(
                FmbRouteFamily::LINEAR, HALO_ACTION_DOWN_LINEAR_SITE,
                static_cast<int64_t>(FmbLinearRouteSelector::AUTO_TILE),
                /*resolved_flags=*/0,
                {seq_len, h, intermediate_size(), 0, NUM_CORES, 0},
                route_invocation);
    ...
    if (ctx().has_complete_physical_manifest()) {
        ctx().consume_physical_route(
            FmbRouteFamily::ALL_REDUCE,
            HALO_ACTION_MLP_ALL_REDUCE_SITE, ring_selector,
            /*resolved_flags=*/0,
...
std::vector<int64_t> rpu_halo_action_expert_planner_cache_identity(int64_t handle) {
    return HaloActionExpertRegistry::get(handle, "rpu_halo_action_expert_planner_cache_identity")
...
void rpu_halo_action_expert_bind_kvinsert_costs(
    ...
    HaloActionExpertRegistry::get(handle, "rpu_halo_action_expert_bind_kvinsert_costs")
...
rpu_halo_action_expert_kvinsert_exact_candidate(
    ...
    return HaloActionExpertRegistry::get(handle, "rpu_halo_action_expert_kvinsert_exact_candidate")
...
KvInsertCostDomainQuery rpu_halo_action_expert_kvinsert_cost_domain(
    ...
    return HaloActionExpertRegistry::get(handle, "rpu_halo_action_expert_kvinsert_cost_domain")
        ->kvinsert_cost_domain("halo_action_expert", descriptor);
}

std::string rpu_halo_action_expert_kvinsert_cost_catalog_sha256(int64_t handle) {
    return HaloActionExpertRegistry::get(
        handle, "rpu_halo_action_expert_kvinsert_cost_catalog_sha256")
...
void rpu_halo_action_expert_set_chunk_envelope(
    ...
    TORCH_CHECK(chunk_size == 32,
                "halo_action_expert_set_chunk_envelope: fixed chunk must be 32, got ",
                chunk_size);
    HaloActionExpertRegistry::get(
        handle, "rpu_halo_action_expert_set_chunk_envelope")
...
std::vector<int64_t> rpu_halo_action_expert_resolve_stage_domain(
    ...
    return HaloActionExpertRegistry::get(
        handle, "rpu_halo_action_expert_resolve_stage_domain")
