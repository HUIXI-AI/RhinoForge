...
//
// M1/M2 边界: 本 commit 输入是 host 已合成的 hidden_states x_emb [1, cs, 1536]
// fp16 — boundary 投影 (action in/out proj、time embedding) 仍在 host (M1)。
// M2 才把 boundary 入模。proof 边界: 本 commit 只编译为目标 (skeleton + 层体),
// 尚无 RPU 硬件正确性证据 (not-proven)。
//
...
public:
    ...
    // Bind the builder-validated cache capacity and the fixed C32 action
    // geometry before first dispatch.  The native stage-domain query below is
    // then the same authority consumed by step_forward.
...
// C API 自由函数原型集中在 src/core/rpu_kernel_decls.h (HALO action expert 段),
