...
// (Bagel._forward_flow, source/HALO-main/modeling/bagel/bagel.py:1200-1333) 对
...
//
//  1. **双流 MoT(非单流)。** golden 证伪了早先"单流"假设: query 含 2 个 text 行
//     (packed_text_indexes=[0,1377], packed_text_ids=[151652,151653]=start/end_of_image)。
//     BAGEL `Qwen2MoTAttention.forward_inference` (qwen2_navit.py:1815-1921) +
//     `Qwen2MoTDecoderLayer` (:2014-2099): 2 个 text 行走 **base(无后缀)und 孪生**
//     (`q_proj`/`q_norm`/`input_layernorm`/`mlp`/`norm`), 1376 vae 行走 **`*_moe_gen`
//     孪生**; per-twin 投影后合并成单一 Q/K/V → 单次 RoPE → 单次非因果全 attend SDPA。
...
// chunk.offset)。emit_row_merge / mlp_half 从 action expert 逐字 lift。
//
// M1/M2 边界: 输入是 host 已合成的 x_emb [1, 1378, 1536] fp16 (vae2llm/time_embedder/
...
public:
    ...
    void set_execution_envelope(int64_t max_kv_len);
    ...
    //   读 (fused_model_base.cpp:1066-1086 契约)。host 随后 llm2vae → slice 1376 vae
    //   行 → velocity。
...
protected:
    ...
    // 根因订正: mask=1(LTM, 抄自因果 action expert) 的 sQryAcc%16 约束对 CFG
    // prefix_len(9586/9579/7, 非 16 倍数) 恒假, 误拒全部 cs。
...
private:
    ...
    std::vector<int64_t> kvinsert_cost_weight_identity() const override {
        ...
        append_kvinsert_cost_scalar_identity(identity, eps_halo_);
        identity.insert(identity.end(), {
            static_cast<int64_t>(has_qkv_bias_halo_)});
        identity.push_back(static_cast<int64_t>(und_layers_.size()));
        for (const auto& weights : und_layers_) {
            for (const auto* tensor : {
                    ...
                    &weights.k_bias, &weights.v_bias}) {
                append_kvinsert_cost_tensor_identity(identity, *tensor);
            }
        }
        identity.push_back(static_cast<int64_t>(gen_layers_.size()));
        for (const auto& weights : gen_layers_) {
            for (const auto* tensor : {
                    &weights.q_w, &weights.k_w, &weights.v_w, &weights.o_w,
                    &weights.q_norm_w, &weights.k_norm_w, &weights.input_norm_w, &weights.post_norm_w,
                    &weights.gate_w, &weights.up_w, &weights.down_w, &weights.q_bias,
        ...
        for (const auto* tensor : {
                &final_norm_w_halo_, &final_norm_gen_w_}) {
            append_kvinsert_cost_tensor_identity(identity, *tensor);
        }
        return identity;
    }
    ...
    // set_weights 改 head 维度的复用场景 (cold-panel re-review:同 qwen3/siglip 的修复,
    // image_flow 是第三个本 MR 引入此模式的模型,旧单 q_ddr_buf_ 守卫只查 NUM_CORES+seq_len
    // 从不查宽度、且 grow-only on seq → 同款 stale-replay/OOB)。镜像 q_ddr_slots_/
    // temp_ddr_slots_;per-shape slot 为 replay 稳定不可释放,128 软上限拦 runaway。
