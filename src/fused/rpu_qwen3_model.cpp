// rpu_qwen3_model.cpp — Qwen3 all-layers-once model (v3 FusedModelBase port)
//
// Plan 01-02: ports Qwen3Model from the v2 two-class framework pair to
// the new flat v3 FusedModelBase single-class (from Plan 01-01). The port is
// mechanical — same compute ops, same SPM layout, same weight binding order,
// same 8-phase pipeline. The only changes vs v2 are framework-facing:
//   - Inherit from v3::FusedModelBase (lives in namespace v3).
//   - Remove all v2-only virtual overrides (kv_insert / preload / post / alloc /
//     layout-changed / persistent-invalidated / chunk-size / temp-total hooks)
//     — all absent from v3 subclass contract per D-203.
//   - Use addr_offset(name).value at SDPA + KV-insert call sites
//     (Pitfall 3 structural fix via typed SpmOffset).
//   - Access model params via pimpl getters (num_layers() / hidden_size() / ...).
//   - set_weights / set_lm_head / clear_lm_head END with invalidate_model_state()
//     per D-503 per-function awk contract.
//
...
#include "rpu_qwen3_model.h"  // extracted v3::CausalDecoderModel + constants (Task 3)
...
// CAUSAL_DECODER_CPP_FIXED_KERNEL_BASIS: this translation unit contains only
// invariant handle/runtime glue outside the descriptor routes declared in the
// model header; it exposes no physical candidate selector.
...
SpmPipelineComponentLayout CausalDecoderModel::prepare_wall_oss_z1_layout(
    ...
    std::vector<int64_t> stage_descriptor =
        encode_causal_physical_stage_descriptor(
            stage_plan, layout,
            /*input_chunk_size=*/stage_plan.input.plan.chunk_size,
            /*qkv_chunk_size=*/resolved_chunk_size,
            /*compute_chunk_size=*/resolved_chunk_size,
            execution_len, real_len, /*position=*/0,
            /*rope_mode=*/1, FmbGraphLifecycle::COMPOSITE_CHILD,
            CausalDecoderPlanningMode::WALL_Z1);
    ...
    wall_z1_stage_descriptor_ = std::move(stage_descriptor);
    ka_last_position_ = -1;
    ka_last_seq_ = -1;
...
void CausalDecoderModel::unprepare_wall_oss_z1_layout() {
    ...
    wall_z1_stage_descriptor_.clear();
}
...
at::Tensor CausalDecoderModel::forward_wall_oss_z1(
    ...
    try {
        TORCH_CHECK(!wall_z1_stage_descriptor_.empty(),
                    "Wall-OSS text Z1 physical descriptor is missing");
...
CausalDecoderModel::prepare_qwen3vl_pooler_z1_layout(
    int64_t execution_len,
    int64_t real_len,
    int64_t retained_deepstack_count, at::IntArrayRef selected_descriptor) {
    TORCH_CHECK(!RpuKernelGraph::has_active(),
    ...
    layout.use_attn_mask = false;
    layout.is_causal = true;
    const auto stage_plan = compose_fmb_default_three_stage_chunk_plan(
        layout, execution_len, /*position=*/0, ChunkMode::SEQUENTIAL);
    if (!selected_descriptor.empty()) {
        (void)kvinsert_cost_domain("causal_decoder", selected_descriptor);
        const auto prepared_candidate = prepare_stage_candidate(selected_descriptor);
        const auto& candidate = prepared_candidate->candidate();
        const auto& manifest = candidate.physical_manifest;
        const bool partial_mrope = std::any_of(manifest.routes.begin(), manifest.routes.end(),
            [](const auto& route) {
                return route.family == FmbRouteFamily::ROPE &&
                    route.site_id == CAUSAL_DECODER_PARTIAL_MROPE_TENSOR_SITE;
            });
        TORCH_CHECK(kvinsert_cost_owner_prefix(selected_descriptor) ==
                        (std::vector<int64_t>{INT64_C(0x51565a3154434f53),
                            real_len, retained_deepstack_count, partial_mrope ? 1 : 0}),
                    "Z1 Text selection was not produced by its exact native lifecycle");
        const auto stage_fingerprint = fmb_three_stage_chunk_plan_fingerprint(stage_plan);
        TORCH_CHECK(candidate.input_chunk_size == 128 && candidate.qkv_chunk_size == 128 &&
                        candidate.compute_chunk_size == 128 &&
                        fmb_three_stage_chunk_plan_fingerprint(candidate.stage_plan) == stage_fingerprint &&
                        manifest.state == FmbPhysicalManifestState::COMPLETE &&
                        manifest.graph_lifecycle == FmbGraphLifecycle::COMPOSITE_CHILD &&
                        manifest.logical_length == real_len && manifest.physical_length == execution_len &&
                        (!partial_mrope || (real_len == QWEN3VL_POOLER_Z1_CANARY_REAL_LEN &&
                                           retained_deepstack_count == 3)),
                    "Qwen3-VL pooler Z1 selected descriptor differs from actual shape/RoPE/lifecycle");
        layout.stage_plan_fingerprint = stage_fingerprint;
        layout = bind_physical_layout_context(layout, manifest);
    }
    ...
    qwen3vl_pooler_z1_stage_descriptor_.clear();
    qwen3vl_pooler_z1_selected_stage_descriptor_.assign(
        selected_descriptor.begin(), selected_descriptor.end());
    ka_last_position_ = -1;
    ka_last_seq_ = -1;
    ka_last_pos_src_ = nullptr;
    ka_last_pos_src_version_ = -1;
...
void CausalDecoderModel::prime_qwen3vl_pooler_z1_inputs(
    ...
    // Validate the prepared winner before mutable keepalive DMA or cache state.
    const auto stage_descriptor = qwen3vl_pooler_z1_stage_descriptor(partial_mrope);
    const uint64_t position_ids_addr =
        RpuGetDevAddr(position_ids.data_ptr<int32_t>());
    ...
    qwen3vl_pooler_z1_stage_descriptor_ = stage_descriptor;
    qwen3vl_pooler_z1_inputs_primed_ = true;
}

std::vector<int64_t> CausalDecoderModel::qwen3vl_pooler_z1_stage_descriptor(
    bool partial_mrope) {
    TORCH_CHECK(!RpuKernelGraph::has_active() && !qwen3vl_pooler_z1_dispatch_,
                "Qwen3-VL pooler Z1 descriptor must resolve outside dispatch");
    validate_qwen3vl_pooler_z1_contract();
    const std::vector<int64_t> owner_prefix{
        INT64_C(0x51565a3154434f53), qwen3vl_pooler_z1_prepared_real_len_,
        qwen3vl_pooler_z1_retained_deepstack_count_, partial_mrope ? 1 : 0};
    if (!qwen3vl_pooler_z1_selected_stage_descriptor_.empty()) {
        (void)kvinsert_cost_domain("causal_decoder", qwen3vl_pooler_z1_selected_stage_descriptor_);
        TORCH_CHECK(kvinsert_cost_owner_prefix(qwen3vl_pooler_z1_selected_stage_descriptor_) == owner_prefix,
                    "Qwen3-VL pooler Z1 prepared selection has a different RoPE/lifecycle");
        return qwen3vl_pooler_z1_selected_stage_descriptor_;
    }
    LayoutContext layout;
    layout.chunk_size = QWEN3VL_POOLER_Z1_CANARY_CHUNK_SIZE;
    layout.max_kv_seq_len = QWEN3VL_POOLER_Z1_CANARY_EXECUTION_LEN;
    ...
    const FmbThreeStageChunkPlan stage_plan =
        compose_fmb_default_three_stage_chunk_plan(
            layout, QWEN3VL_POOLER_Z1_CANARY_EXECUTION_LEN,
            /*position=*/0, ChunkMode::SEQUENTIAL);
    auto descriptor = encode_causal_physical_stage_descriptor(
            stage_plan, layout,
            /*input_chunk_size=*/stage_plan.input.plan.chunk_size,
            /*qkv_chunk_size=*/QWEN3VL_POOLER_Z1_CANARY_CHUNK_SIZE,
            /*compute_chunk_size=*/QWEN3VL_POOLER_Z1_CANARY_CHUNK_SIZE,
            QWEN3VL_POOLER_Z1_CANARY_EXECUTION_LEN,
            qwen3vl_pooler_z1_prepared_real_len_, /*position=*/0,
            /*rope_mode=*/partial_mrope ? 1 : 0,
            FmbGraphLifecycle::COMPOSITE_CHILD,
            CausalDecoderPlanningMode::QWEN3VL_POOLER_Z1);
    bind_kvinsert_cost_owner_prefix(descriptor, owner_prefix);
    return descriptor;
}
...
void CausalDecoderModel::unprepare_qwen3vl_pooler_z1_layout() {
    ...
    qwen3vl_pooler_z1_selected_stage_descriptor_.clear();
...
at::Tensor CausalDecoderModel::forward_qwen3vl_pooler_z1(
    ...
    try {
        TORCH_CHECK(!qwen3vl_pooler_z1_stage_descriptor_.empty(),
                    "Qwen3-VL pooler Z1 physical descriptor is missing");
        at::Tensor result = forward(
            hidden_shape_carrier, k_caches, v_caches,
            /*attention_mask=*/std::nullopt,
            /*position=*/0,
            ...
            /*allow_batch_decode=*/false,
            /*planned_chunk_size=*/0,
            qwen3vl_pooler_z1_stage_descriptor_);
        TORCH_CHECK(get_last_resolved_chunk_size() ==
                        QWEN3VL_POOLER_Z1_CANARY_CHUNK_SIZE,
...
std::vector<int64_t> CausalDecoderModel::bind_multiview_text_stage_descriptor(
    const FmbThreeStageChunkPlan& stage_plan, LayoutContext& layout,
    bool partial_mrope, at::IntArrayRef selected_descriptor) {
    const std::vector<int64_t> owner_prefix{
        INT64_C(0x51564d5654434f53), partial_mrope ? 1 : 0};
    std::vector<int64_t> descriptor;
    if (selected_descriptor.empty()) {
        descriptor = encode_causal_physical_stage_descriptor(
            stage_plan, layout, stage_plan.input.plan.chunk_size,
            QWEN3VL_MULTIVIEW_TEXT_RESOLVED_CHUNK_SIZE,
            QWEN3VL_MULTIVIEW_TEXT_RESOLVED_CHUNK_SIZE,
            QWEN3VL_MULTIVIEW_TEXT_EXECUTION_LEN,
            QWEN3VL_MULTIVIEW_TEXT_EXECUTION_LEN, /*position=*/0,
            /*rope_mode=*/partial_mrope ? 1 : 0,
            FmbGraphLifecycle::COMPOSITE_CHILD,
            CausalDecoderPlanningMode::QWEN3VL_MULTIVIEW_TEXT);
        bind_kvinsert_cost_owner_prefix(descriptor, owner_prefix);
    } else {
        // Only this owner's current oracle (including its genuine EXACT mint)
        // may supply a selection. Never rebuild AUTO over a calibrated route.
        (void)kvinsert_cost_domain("causal_decoder", selected_descriptor);
        TORCH_CHECK(kvinsert_cost_owner_prefix(selected_descriptor) == owner_prefix,
                    "multiview Text selection was not produced by its exact native lifecycle");
        descriptor.assign(selected_descriptor.begin(), selected_descriptor.end());
    }
    ...
    const int64_t rope_site = partial_mrope
        ? CAUSAL_DECODER_PARTIAL_MROPE_TENSOR_SITE
        : CAUSAL_DECODER_MROPE_TENSOR_SITE;
    TORCH_CHECK(
        candidate.input_chunk_size == stage_plan.input.plan.chunk_size &&
            candidate.qkv_chunk_size == QWEN3VL_MULTIVIEW_TEXT_RESOLVED_CHUNK_SIZE &&
            candidate.compute_chunk_size == QWEN3VL_MULTIVIEW_TEXT_RESOLVED_CHUNK_SIZE &&
            fmb_three_stage_chunk_plan_fingerprint(candidate.stage_plan) == stage_fingerprint &&
            manifest.state == FmbPhysicalManifestState::COMPLETE &&
            manifest.graph_lifecycle == FmbGraphLifecycle::COMPOSITE_CHILD &&
            manifest.logical_length == QWEN3VL_MULTIVIEW_TEXT_EXECUTION_LEN &&
            manifest.physical_length == QWEN3VL_MULTIVIEW_TEXT_EXECUTION_LEN &&
            std::any_of(manifest.routes.begin(), manifest.routes.end(),
                [rope_site](const auto& route) {
                    return route.family == FmbRouteFamily::ROPE && route.site_id == rope_site;
                }),
        "Qwen3-VL multiview Text selected descriptor differs from actual shape/RoPE/lifecycle");
    layout.stage_plan_fingerprint = stage_fingerprint;
    layout = bind_physical_layout_context(layout, manifest);
    return descriptor;
}
...
CausalDecoderModel::prepare_qwen3vl_multiview_text_composite_layout(
    ...
    auto descriptor = bind_multiview_text_stage_descriptor(
        shape.stage_plan, shape.allocation_layout,
        /*partial_mrope=*/!selected_descriptor.empty(), selected_descriptor);
    const SpmPipelineComponentLayout prepared =
        prepare_spm_pipeline_component(
            shape.allocation_layout, shape.stage_plan);
    TORCH_CHECK(
        prepared.temporary_bytes ==
    ...
    qwen3vl_multiview_text_composite_allocation_chunk_size_ =
        shape.allocation_layout.chunk_size;
    qwen3vl_multiview_text_stage_descriptor_ = std::move(descriptor);
    rollback.release();
    return prepared;
}
...
CausalDecoderModel::resolve_qwen3vl_multiview_text_composite_profile() {
    ...
    request.version = QWEN3VL_MULTIVIEW_TEXT_PROFILE_VERSION;
...
void CausalDecoderModel::prime_qwen3vl_multiview_text_inputs(
    ...
    const bool partial_mrope = rope_cos_il.has_value();
    LayoutContext layout;
    layout.chunk_size = QWEN3VL_MULTIVIEW_TEXT_ALLOCATION_CHUNK_SIZE;
    layout.max_kv_seq_len = QWEN3VL_MULTIVIEW_TEXT_EXECUTION_LEN;
    layout.num_layers = num_layers();
    layout.use_attn_mask = false;
    layout.is_causal = true;
    const auto stage_plan = compose_fmb_default_three_stage_chunk_plan(
        layout, execution_len, /*position=*/0, ChunkMode::SEQUENTIAL);
    TORCH_CHECK(!qwen3vl_multiview_text_stage_descriptor_.empty(),
                "Qwen3-VL multiview Text prime requires its prepared descriptor");
    (void)bind_multiview_text_stage_descriptor(
        stage_plan, layout, partial_mrope, qwen3vl_multiview_text_stage_descriptor_);
...
void CausalDecoderModel::rollback_qwen3vl_multiview_text_inputs() {
    ...
    qwen3vl_multiview_text_stage_descriptor_.clear();
    partial_mrope_active_ = false;
    ka_last_position_ = -1;
    ka_last_seq_ = -1;
    ka_last_pos_src_ = nullptr;
...
at::Tensor CausalDecoderModel::forward_qwen3vl_multiview_text_composite(
    ...
    TORCH_CHECK(!qwen3vl_multiview_text_stage_descriptor_.empty(),
                "Qwen3-VL multiview Text physical descriptor is missing");
    at::Tensor result = forward(
        ...
        /*allow_batch_decode=*/false,
        /*planned_chunk_size=*/0,
        qwen3vl_multiview_text_stage_descriptor_);
    TORCH_CHECK(
        get_last_resolved_chunk_size() ==
            QWEN3VL_MULTIVIEW_TEXT_RESOLVED_CHUNK_SIZE,
...
CausalDecoderModel::prepare_qwen3vl_multiview_text_dry_layout(
    ...
    TORCH_CHECK(
        ...
        "allocation C225 causal-resident plan");
    auto descriptor = bind_multiview_text_stage_descriptor(
        shape.stage_plan, shape.allocation_layout, /*partial_mrope=*/true);
    ...
    qwen3vl_multiview_text_dry_allocation_chunk_size_ =
        shape.allocation_layout.chunk_size;
    rollback.release();
    return {std::move(shape), component, std::move(descriptor)};
}
...
std::vector<int64_t> rpu_causal_decoder_planner_cache_identity(int64_t handle) {
    return CausalDecoderRegistry::get(handle, "rpu_causal_decoder_planner_cache_identity")
...
SpmPipelineComponentLayout prepare_text(
    int64_t handle,
    int64_t execution_len,
    int64_t real_len, at::IntArrayRef selected_descriptor) {
    return CausalDecoderRegistry::get(
               handle, "qwen3vl_pooler_z1_prepare_text")
        ->prepare_qwen3vl_pooler_z1_layout(
            execution_len, real_len, /*retained_deepstack_count=*/0, selected_descriptor);
}
...
SpmPipelineComponentLayout prepare_text_deepstack1(
    ...
    return CausalDecoderRegistry::get(
        ...
        ->prepare_qwen3vl_pooler_z1_layout(
            execution_len, real_len, /*retained_deepstack_count=*/1, selected_descriptor);
}
...
SpmPipelineComponentLayout prepare_text_deepstack3(
    ...
    return CausalDecoderRegistry::get(
        ...
        ->prepare_qwen3vl_pooler_z1_layout(
            execution_len, real_len, /*retained_deepstack_count=*/3, selected_descriptor);
}

std::vector<int64_t> text_stage_descriptor(int64_t handle, bool partial_mrope) {
    return CausalDecoderRegistry::get(handle, "qwen3vl_pooler_z1_text_stage_descriptor")
        ->qwen3vl_pooler_z1_stage_descriptor(partial_mrope);
}
...
SpmPipelineComponentLayout prepare_multiview_text_composite(
    ...
    return CausalDecoderRegistry::get(
        ...
        ->prepare_qwen3vl_multiview_text_composite_layout(execution_len, selected_descriptor);
}
...
std::vector<int64_t> multiview_text_stage_descriptor(int64_t handle) {
    return CausalDecoderRegistry::get(handle, "qwen3vl_multiview_text_stage_descriptor")
        ->multiview_text_stage_descriptor();
}
...
int64_t rpu_causal_decoder_create(
    bool qwen3_spm_kv_by_mha, bool adarms_fused_bcast, int64_t num_cores,
    int64_t vocab_size) {
    TORCH_CHECK(num_cores == 8 || num_cores == 6 || num_cores == 4,
                "causal decoder admits the 4/6/8 execution-core budgets");
    v3::resolve_causal_decoder_lm_head_tp(static_cast<int>(num_cores), vocab_size);
    const int64_t handle = CausalDecoderRegistry::create();
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_create")
        ->configure_cold_routes(
            qwen3_spm_kv_by_mha, adarms_fused_bcast, static_cast<int>(num_cores),
            vocab_size);
    return handle;
}

std::vector<int64_t> rpu_causal_decoder_topology(int64_t handle) {
    return CausalDecoderRegistry::get(handle, "rpu_causal_decoder_topology")
        ->execution_topology();
}
...
void rpu_causal_decoder_destroy(int64_t handle) {
    ...
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_destroy")
        ->check_execution_reconfigure_destroy_allowed(
            "rpu_causal_decoder_destroy");
...
void rpu_causal_decoder_set_lm_head(
    ...
    bool fuse_prefill_last,
    int64_t exact_candidates) {
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder")
        ->set_lm_head(
            lm_head_w, lm_head_scale, fuse_prefill_last, exact_candidates);
}
...
void rpu_causal_decoder_set_rhinovla_high_precision(int64_t handle, bool enabled) {
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_set_rhinovla_high_precision")
...
void rpu_causal_decoder_set_rhinovla_high_precision_fusions(int64_t handle, bool enabled) {
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_set_rhinovla_high_precision_fusions")
        ->set_rhinovla_high_precision_fusions(enabled);
}
...
// MR-D: the per-handle replacement for the deleted process-global set_chunk_size.
//
// Legacy handles retain the signature-keyed hot path. Production owners opt in
// to the execution-reconfigure guard before first dispatch and must then stage a
// new generation through the opaque coordinator token.
void rpu_causal_decoder_set_chunk_size_override(int64_t handle, int64_t chunk_size) {
    ...
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_set_chunk_size_override")
        ->set_control_chunk_size_override(
            chunk_size, "rpu_causal_decoder_set_chunk_size_override");
}

void rpu_causal_decoder_set_explicit_mask_spm_residency(int64_t handle, bool enabled) {
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_set_explicit_mask_spm_residency")
        ->set_explicit_mask_spm_residency(enabled);
}

void rpu_causal_decoder_enable_execution_reconfigure(int64_t handle) {
    CausalDecoderRegistry::get(
        handle, "rpu_causal_decoder_enable_execution_reconfigure")
...
void rpu_causal_decoder_bind_kvinsert_costs(
    ...
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_bind_kvinsert_costs")
...
KvInsertCostDomainQuery rpu_causal_decoder_kvinsert_cost_domain(
    ...
    return CausalDecoderRegistry::get(handle, "rpu_causal_decoder_kvinsert_cost_domain")
        ->kvinsert_cost_domain("causal_decoder", descriptor);
}

std::string rpu_causal_decoder_kvinsert_cost_catalog_sha256(int64_t handle) {
    return CausalDecoderRegistry::get(
        handle, "rpu_causal_decoder_kvinsert_cost_catalog_sha256")
...
void rpu_causal_decoder_stage_chunk_size_override(
    ...
    TORCH_CHECK(token > 0,
                "causal-decoder hot-reconfigure token must be positive");
    CausalDecoderRegistry::get(
        handle, "rpu_causal_decoder_stage_chunk_size_override")
        ->stage_control_chunk_size_override(
            static_cast<uint64_t>(token), chunk_size,
            "rpu_causal_decoder_stage_chunk_size_override");
}
...
// MR-A: declare this handle's CERTIFIED chunk envelope. Deny-by-default —
// without it the planner refuses to prefill. Allowlist + evidence rules:
    ...
    at::IntArrayRef planned_stage_descriptor)
{
    ...
    return CausalDecoderRegistry::get(handle, "rpu_causal_decoder")->forward(
        ...
        cos_sin_offset, batch_slot, allow_batch_decode, planned_chunk_size,
...
{
    CausalDecoderRegistry::get(handle, "rpu_causal_decoder_prepare_persistent_spm")
        ->prepare_persistent_spm(
            execution_len, position, is_causal, mask_kv_len);
}
...
std::vector<int64_t> rpu_causal_decoder_resolve_prefill_stage_domain(
    ...
    return CausalDecoderRegistry::get(
        handle, "rpu_causal_decoder_resolve_prefill_stage_domain")
        ->resolve_prefill_stage_domain(
            execution_len, position, is_causal, mask_kv_len, rope_mode,
            graph_lifecycle, logical_len, planning_chunk_size_override);
}
...
rpu_causal_decoder_kvinsert_exact_candidate(
    ...
    return CausalDecoderRegistry::get(handle, "rpu_causal_decoder_kvinsert_exact_candidate")
...
std::vector<int64_t> rpu_causal_decoder_resolve_decode_stage_descriptor(
    int64_t handle, int64_t position) {
    return CausalDecoderRegistry::get(
        handle, "rpu_causal_decoder_resolve_decode_stage_descriptor")
        ->resolve_decode_stage_descriptor(position);
}
