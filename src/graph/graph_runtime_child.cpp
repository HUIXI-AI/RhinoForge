...
static std::unique_ptr<::rhino_lkn::Kernel_t> clone_kernel_with_args(
    ...
    // Mirrors rhino-launch-kernel's R1 cfg layout. cfg slots 0..34 are backed
    ...
    if (node.kernel_id.has_value()) {
        kernel_name = KERNEL_ID_NAMES[static_cast<size_t>(*node.kernel_id)];
    } else if (!node.kernel_name.empty()) {
...
void RpuKernelGraph::register_child(std::shared_ptr<RpuKernelGraph> child,
    ...
    TORCH_CHECK(
        ...
        "register_child: child execution_core_count exceeds parent budget");
...
void RpuKernelGraph::record_branch(uint64_t branch_key,
    ...
    TORCH_CHECK(label != BranchNodeData::physical_manifest_label(),
                "record_branch: reserved physical manifest label is forbidden");
...
void RpuKernelGraph::record_physical_manifest_branch(
        uint64_t manifest_fingerprint) {
    TORCH_CHECK(manifest_fingerprint != 0,
                "COMPLETE physical manifest Branch needs a nonzero identity");
    const bool census = kernel_register_census_active_ ||
        has_kernel_register_census_nodes_ ||
        pending_kernel_register_census_.has_value();
    TORCH_CHECK(state_ == State::RECORDING || state_ == State::REPLAYING,
                "COMPLETE physical manifest Branch requires capture/replay");
    if (census) {
        TORCH_CHECK(kernel_register_census_active_ &&
                    ...
                    has_active() && &active() == this &&
                    (state_ == State::RECORDING || state_ == State::REPLAYING),
                "COMPLETE physical manifest Branch requires a current, "
                "unpoisoned register-census invocation without a pending Kernel");
        check_kernel_register_dma_burst_closed("physical manifest Branch");
    }
    BranchNodeData data;
    data.branch_key = manifest_fingerprint;
    data.label = BranchNodeData::physical_manifest_label();
    // Keep the ordinary COMPLETE child-window replay path. Public capture_data
    // rejects this reserved label; only this FMB-friend entry can mint it.
    if (!census && capture_replay_child_skip_data(data)) return;
    if (state_ == State::REPLAYING) {
        TORCH_CHECK(cursor_ < nodes_.size() &&
                        nodes_[cursor_].kind == GraphNodeKind::Branch,
                    "COMPLETE physical manifest Branch replay node mismatch");
        const auto& recorded = nodes_[cursor_].as_branch();
        TORCH_CHECK(recorded.branch_key == manifest_fingerprint &&
                        recorded.branch_signature == GraphSignature{} &&
                        recorded.label == data.label,
                    "COMPLETE physical manifest Branch replay identity mismatch");
    }
    // Count the real metadata node in the exact ordered census. It contains
    // no registers or DMA addresses and never enters relocation catalogs.
    if (census) observe_physical_manifest_branch_for_census();
    if (state_ == State::RECORDING) {
        nodes_.push_back(GraphNode{GraphNodeKind::Branch, std::move(data)});
    } else {
        ++cursor_;
    }
}
...
std::shared_ptr<RpuKernelGraph> RpuKernelGraph::extract_child_window(
    ...
    TORCH_CHECK(
        false,
        "extract_child_window is unavailable with rhino-launch-kernel "
        "v1.0.0 Release because raw kernel cloning is a development API");
    return nullptr;
#else
    RpuExecutionCoordinator::require_no_physical_claim(
    ...
    auto child = std::make_shared<RpuKernelGraph>(runtime_policy_);
    ...
    // P8.0 修复:Queue_t 不再 per-segment 持有 (走 QueueCache 全局缓存)。
    // 此处不再预 build prepared_wq;child replay 时 launch_segment_for_replay
...
void RpuKernelGraph::replace_window_with_child(
    ...
    TORCH_CHECK(
        child->runtime_policy_.execution_core_count <=
            runtime_policy_.execution_core_count,
        "replace_window_with_child: child execution_core_count exceeds parent budget");
